...
if flg==0:
    naVal = df.mean(numeric_only=True) # 欠損値を置き換える処理
    df = df.fillna(naVal) #欠損値補完
elif flg ==1:
    naVal = df.median(numeric_only=True) # 欠損値を置き換える処理
    df = df.fillna(naVal) #欠損値補完
else:
    #特殊処理記述
        pass
...
def lean(x, t, depoth):
    ...
    model = tree.DecisionTreeClassifier(max_depth=depoth, random_state=0)
    model.fit(x_t,y_t)
